...
📈Выработка - <b>${findOperator.fact_call_255}</b> / ${
    findOperator.plan_call_255
  }
🕗Ср.вр разговора - <b>${findOperator.fact_avarage_call_255}</b> / ${
    findOperator.plan_avarage_call_255
  }
...
const updateDatabase = async (msg) => {
  const chatId = msg.from.id;
  // console.log('okk');
  const deletedData = await DeleteDatabase(msg);
